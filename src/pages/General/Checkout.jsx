...
const Checkout = () => {
    ...
    const initialOptions = {
        clientId: "AcMqXNtjE44JT6QyytYDBaF-kzZHSCjwZLv2EfMbuavqDtxibI-q-Sm0J6nkCLw-Szx95PaA8mI1MQ5b",
        currency: "USD",
        intent: "capture",
    };
        ...
        axiosClient.post('/orders', {
            ...
            .then(res => {
                axiosClient.post('/transaction', {
                    ...
                    country_code: order.payer.address.country_code ? order.payer.address.country_code : 'US',
            ...
            .then((res) => {
                axiosClient.post('/carts/clear', {
                    ...
                    emailjs.send('service_090fxwg', 'template_rmutrmo', templateParams, 'oZzkqsH6bTI3yU7ac')
                        .then(function (response) {
                            console.log('SUCCESS!', response.status, response.text);
                            navigate(`/checkout/${window.localStorage.getItem('device')}/thank-you`)
                        }, function (error) {
            ...
            <div className="row" style={{
                ...
                <div className="row" style={{
                    ...
                    <div className="col-lg-6" >
                        <div className="row"><h2>Thông tin đơn hàng</h2></div>
                        <form action="" method="post" className="row">
                            <input name="fullname" type="text" placeholder="Họ và Tên" onChange={(e) => {
                                setOrder({ ...order, full_name: e.target.value })
                            }} />
                            <input name="email" type="text" placeholder="Email" onChange={(e) => {
                                setOrder({ ...order, email: e.target.value })
                            }} />
                            <input name="phone" type="text" placeholder="Số điện thoại" onChange={(e) => {
                                setOrder({ ...order, phone: e.target.value })
                            }} />
                            <input name="address" type="text" placeholder="Địa chỉ" onChange={(e) => {
                                ...
                                return (
                                    <div style={{
                                        ...
                                        <input
                                            ...
                                            onClick={(e) => {
                                                document.querySelectorAll('.shipping-checkbox').forEach(s => {
                        ...
                        <form>
                            {payment.map(p => <div style={{
                                display: 'flex'
                            }}>
                                {p.type === 'cod'
                                    || p.type === 'Banking'
                                    || p.type === 'At Store' ? <input
                                    type="checkbox"
                                    ...
                                    onClick={(e) => {
                                        document.querySelectorAll('.payment-checkbox').forEach(st => {
                                            if (st !== e.currentTarget) st.checked = false
                                        })
                                        setOrderMethod({ ...orderMethod, payment: p.id })
                                    }}
                                /> : ''}
                                {p.type === 'cod' ? <label htmlFor={p.id} style={{
                                    display: 'flex',
                                    justifyContent: 'space-between',
                                    width: '70%'
                                }}>
                                ...
                                {p.type === 'Banking' ? <label htmlFor={p.id} style={{
                                    display: 'flex',
                                    justifyContent: 'space-between',
                                    width: '70%'
                                }}>
                                    <span>{p.type}</span>
                                    <span>{JSON.parse(p.detail).bank_name + ': ' + JSON.parse(p.detail).number}</span>
                                </label> : ''}
                                {p.type === 'At Store' ? <label htmlFor={p.id} style={{
                                    display: 'flex',
                                    justifyContent: 'space-between',
                                    width: '70%'
                                }}>
                                    <span>{p.type}</span>
                                    <span>{JSON.parse(p.detail).name + ': ' + JSON.parse(p.detail).address}</span>
                                </label> : ''}
                            </div>
                            )}
                        </form>
                    ...
                    <div className="col-lg-5">
                        ...
                        <div style={{
                            ...
                            <div style={{
                                ...
                                <input id="total-price" type="number" style={{
                                    display: 'none'
                                }} value={total} />
                            </div>
                        </div>
                            ...
                            <button
                                ...
                                onClick={() => {
                                    axiosClient.post('orders/', {
                                        products: products,
                                        total_price: total,
                                        user_device: window.localStorage.getItem('device'),
