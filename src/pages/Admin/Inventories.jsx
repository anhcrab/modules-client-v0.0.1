          ...
          <thead>
            <tr style={{ backgroundColor: 'white' }}>
              <th style={{ backgroundColor: 'white' }}>
                <select onChange={e => setChoices({ ...choices, type: e.target.value })}>
                  <option value={''}>Types</option>
                  {types.map(type => <option value={type.name}>{type.name}</option>)}
                </select>
              </th>
              <th style={{ backgroundColor: 'white' }}>
                <select onChange={e => setChoices({ ...choices, category: e.target.value })}>
                  <option value={''}>Categories</option>
                  {categories.map(cat => <option value={cat.name}>{cat.name}</option>)}
                </select>
              </th>
              <th style={{ backgroundColor: 'white' }}>
                <select onChange={e => setChoices({ ...choices, size: e.target.value })}>
              ...
              <th style={{ backgroundColor: 'white' }}>
                <select onChange={e => setChoices({ ...choices, color: e.target.value })}>
              ...
              <th style={{ backgroundColor: 'white', display: "flex", alignItems: 'center' }}>
            ...
            <tbody>
              {inventories && filtered.map(p => (
                <tr key={p.id}>
                  ...
                  <td>{p.date}</td>
                  <td>
                    <Link className="btn-edit" to={'/inventories/' + p.id}>Edit</Link>
                    &nbsp;
