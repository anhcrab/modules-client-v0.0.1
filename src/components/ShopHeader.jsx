...
const ShopHeader = ({ cartState, setCartState }) => {
    ...
    useEffect(() => {
        axiosClient.get(`/carts/${window.localStorage.getItem('device')}`)
            .then((res) => {
                ...
                setCount(result)
                setTotal(cart.items && cart.items.reduce((total, item) => {
    ...
    return (
        <header className="row" style={{
            ...
            >logo</Link>
            <nav className="col-lg-8 row" style={{
                display: 'flex',
                textDecoration: 'none',
                color: 'white'
            }}>
            ...
            >
                ...
                <div className={`cart-box row ${hide ? 'hidden' : ''}`} style={{
                    width: '450px',
                    position: 'absolute',
                    right: 0,
                    backgroundColor: '#fff',
                    ...
                    {cart.items && cart.items.map(item => <div
                        className=""
                        style={{
                            width: '100%',
                            display: 'flex',
